	...
	public void insert(TrackingRow tr) {
		...
		for (TrackingRow oldRow : table) {
			if (oldRow.r.isOverlapping(tr.r)) {
				TrackingRow beforeIntersection = new TrackingRow(
						oldRow.r.getLo(), tr.r.getLo() - 1, oldRow.s, oldRow.t);
				TrackingRow afterIntersection = new TrackingRow(
						tr.r.getHi() + 1, oldRow.r.getHi(), oldRow.s, oldRow.t);
		...
		for (TrackingRow toMerge : table) {
			if (toMerge.r.isAdjacent(tr.r) && tr.s == toMerge.s
					&& tr.t == toMerge.t && !(tr.equals(toMerge))) {
